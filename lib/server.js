...
//getters/setters for Clients
PeerServer.prototype.getClient = function (key, id, callback) {
  callback(null, this._clients[key][id]);
}

PeerServer.prototype.getClientKey = function (key, callback) {
  callback(null, this._clients[key]);
}


PeerServer.prototype.getClientCount = function (key, callback) {
  callback(null, this._clients[key].length);
}


PeerServer.prototype.setClient = function (key, id, data, callback) {
  this._clients[key][id] = data;
  callback(null);
}

PeerServer.prototype.setClientKey = function (key, data, callback) {
  this._clients[key] = data;
  callback(null);
}

PeerServer.prototype.removeClient = function (key, id, callback) {
  delete this._clients[key][id];
  callback(null);
}

//getters/setters for IPs
PeerServer.prototype.getIP = function () {

}

PeerServer.prototype.setIP = function () {
  
}

PeerServer.prototype.removeIP = function () {
  
}

//getters/setters for outstandings
PeerServer.prototype.getOutstanding = function () {

}

PeerServer.prototype.setOutstanding = function () {
  
}

PeerServer.prototype.removeOutstanding = function () {
...
PeerServer.prototype._initializeWSS = function() {
  ...
  this._wss.on('connection', function(socket) {
    ...
    async.parallel([
      function (callback) {
        self.getClientKey(key, callback);
      },
      function (callback) {
        self.getClient(key, id, callback);
      }
    ],
    function(err, results){
      if(!results[0] || !results[1]) {
        self._checkKey(key, ip, function(err) {
          if (!err) {
            self.getClient(key, id, function (err, data) {
              if(err) throw err;
              if(!data) {
                self.setClient(key, id, {
                  token: token,
                  ip: ip
                }, function () {
...
PeerServer.prototype._configureWS = function(socket, key, id, token) {
  ...
  self.getClient(key, id, function (err, data) {
    ...
    self._processOutstanding(key, id);
